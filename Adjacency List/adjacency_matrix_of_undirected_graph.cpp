...
4. Read 'e' edges:
   ...
   - Mark adj_mat[a][b] and adj_mat[b][a] as 1 (since the graph is undirected).
...
⚠️ Notes:
- This implementation is for an **undirected graph**, so each edge is stored twice (`adj_mat[a][b]` and `adj_mat[b][a]`).
- **Adjacency matrix is inefficient for large graphs** because it requires **O(n²) space**, making it unsuitable for sparse graphs.
- If a **directed graph** is needed, remove `adj_mat[b][a] = 1`.
*/
